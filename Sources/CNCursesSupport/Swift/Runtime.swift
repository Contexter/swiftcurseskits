...
package enum CNCursesRuntimeError: Error, Equatable {
    ...
    package static func == (lhs: CNCursesRuntimeError, rhs: CNCursesRuntimeError) -> Bool {
        switch (lhs, rhs) {
        case (.localeUnavailable, .localeUnavailable), (.initializationFailed, .initializationFailed):
            return true
        case let (.callFailed(name: lhsName, code: lhsCode), .callFailed(name: rhsName, code: rhsCode)):
